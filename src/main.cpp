...
// Copyright (c) 2013-2014 Cryptotest Developers
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cryptotest: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cryptotest Signed Message:\n";
...
{
    // Cryptotest: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 4 * 60 * 60; // Cryptotest: every 4 hours
static const int64 nTargetSpacing = 60; // Cryptotest: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cryptotest: This fixes an issue where a 51% attack can change difficulty at will.
...
{
    // Cryptotest: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Jippey";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CryptotestMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CryptotestMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CryptotestMiner(CWallet *pwallet)
{
    printf("CryptotestMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cryptotest-miner");
    ...
    try { loop {
        ...
        printf("Running CryptotestMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CryptotestMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CryptotestMiner, pwallet));
}
